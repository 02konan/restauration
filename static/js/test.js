...
function fillDetailsModal(cmd) {
    ...
    if (detailDate) detailDate.textContent = dateFormatee;
    const detailCode = document.getElementById("detailCode");
    if (detailCode) detailCode.textContent = cmd.Numcommande || cmd.id || "N/A";
    const detailClient = document.getElementById("detailClient");
    if (detailClient) detailClient.textContent = cmd.Client || cmd.user || "Client inconnu";
    const detailLieu = document.getElementById("detailLieu");
    if (detailLieu) detailLieu.textContent = cmd.lieu || cmd.adresse || "Inconnu";
    const detailPromo = document.getElementById("detailPromo");
    if (detailPromo) detailPromo.textContent = cmd.code || cmd.promo || "Aucun";
    const detailContact = document.getElementById("detailContact");
    if (detailContact) detailContact.textContent = cmd.telephone || cmd.contact || "N/A";
    const detailQuantite = document.getElementById("detailQuantite");
    if (detailQuantite) detailQuantite.textContent = cmd.quantite || "0";
    const detailTotal = document.getElementById("detailTotal");
    if (detailTotal) detailTotal.textContent = cmd.Total || "0";
    const detailOrderIdTraiter = document.getElementById("detailOrderIdTraiter");
    if (detailOrderIdTraiter) detailOrderIdTraiter.value = cmd.id_commande || cmd.id_produits || "";
    const detailOrderIdLivrer = document.getElementById("detailOrderIdLivrer");
    if (detailOrderIdLivrer) detailOrderIdLivrer.value = cmd.id_commande || cmd.id_produits || "";
