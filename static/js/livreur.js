...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Initialiser l'affichage du cart-livreur
    const cartLivreur = document.getElementById('cart-livreur');
    const listBtn = document.getElementById('list-btn');
    const container = document.getElementById('command-livreur');
    
    if (cartLivreur && listBtn && container) {
        // S'assurer que le cart-livreur est visible au démarrage
        cartLivreur.classList.remove('hidden');
        
        // Utiliser la délégation d'événements pour les boutons "Accepter"
        ...
        // Afficher le cart-livreur quand on clique sur le bouton liste
    ...
    if (!mapElement) return; // Sortir si la carte n'existe pas
...
function affichelivraison(livraison) {
    ...
    // Assurez-vous que la fonction existe avant de l'appeler
    if (typeof initializeDetailsModal === 'function') {
...
`;
        item.addEventListener("click", () => {
            if (typeof fillDetailsModal === 'function') {
                fillDetailsModal(cmd);
            }
        });
...
setTimeout(initializeDeliveryUI, 1000);
